...
- **Fields:** `SUPABASE_URL`, publishable key, service-role key (server only), DB URL, fixed upstream URL, origin HMAC/current+previous key if implemented, cron secret; Vault `edge_functions_base_url` and `account_deletion_worker_secret`; Edge secret `ACCOUNT_DELETION_WORKER_SECRET`. Account-deletion and push worker secrets must be distinct. Use exact names from function code; do not invent aliases.
...
- **Verify:** staging functions/migrations; valid internal call once; replay/invalid HMAC denied. Confirm exactly one production ingestion scheduler and exactly one `etkinlink-account-deletion-continuations` scheduler. Prove that an `auth_deleted` fixture completes after the user JWT is no longer usable and that terminal rows are visible only through the service-only RPC.
- **Rollback:** restore prior secret during overlap, disable new scheduler/route, deploy prior function; never reset production DB.
...
- **Commands:** create/use a protected tag resolving to `<FULL_SHA>`, then `gh workflow run mobile-release.yml --ref <PROTECTED_TAG_AT_FULL_SHA>`; verify the run `head_sha`, download artifacts; `jarsigner -verify -strict <AAB>`; on macOS `codesign --verify --deep --strict <APP>` and inspect entitlements. (`gh workflow run --ref` accepts a branch/tag, not a raw SHA.)
...
- [ ] **Commands:** `gh workflow run mobile-e2e.yml --ref <PROTECTED_TAG_AT_FULL_SHA>`; verify run `head_sha`; then `gh workflow run staging-load-test.yml --ref <PROTECTED_TAG_AT_FULL_SHA> -f confirmation=staging-10k -f target_vus=10000` only after 25/250 gates.
